...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#exampleModal1').on('show.bs.modal', function(e) {
            ...
            $.ajax({
                url: 'budget_get_Request_pending_consideration', // เปลี่ยนเป็นเส้นทางของสคริปต์ PHP ของคุณ
                method: 'GET',
                data: {
                    petition_type: 3, // ตั้งค่าคงที่หากต้องการส่งค่านี้ไปยังเซิร์ฟเวอร์เสมอ
    ...
    $(document).ready(function() {
        ...
        $('#exampleModal4').on('show.bs.modal', function(e) {
            // เรียกใช้งานข้อมูลผ่าน AJAX
            $.ajax({
                url: 'budget_get_all_requests', // เปลี่ยนเป็นเส้นทางของสคริปต์ PHP ของคุณ
                method: 'GET',
                data: {
                    petition_type: 3
    ...
    function fetchTotalPetitions(idStatusArray) {
        $.ajax({
            url: 'budget_get_id_status', // Adjust this URL to the correct endpoint
            method: 'POST',
            data: {
                petition_type: 3 // This needs to be handled on the server-side if it's to be used
            },
            success: function(response) {
