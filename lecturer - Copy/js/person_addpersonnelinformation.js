$(document).ready(function () {
  ...
  $("#server_user").click(function () {
    ...
    $.ajax({
      url: "./api/person_inster_Add_information", // เปลี่ยนเป็น URL ที่ถูกต้องสำหรับไฟล์ PHP ที่จะใช้ในการเพิ่มข้อมูลสินค้า
      method: "POST",
      processData:false,
      contentType:false,
      data: formData,
      ...
      success: function (response) {
        ...
        if (data.status === 200) {
          ...
          window.location.href = "person_addpersonnelinformation";
        } else {
          alert("เกิดข้อผิดพลาดในการบันทึกข้อมูล");
          window.location.href = "person_Add_information";
        }
      },
      error: function () {
