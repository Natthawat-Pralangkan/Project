...
<script>
    if (localStorage.getItem("id_type") != "16" && localStorage.getItem("user_id") == null) {
        localStorage.clear()
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                if (row.id_status == 1) { // ตรวจสอบเฉพาะเมื่อ id_status เป็น 1
                                    ...
                                    buttonHtml = `<button class="btn btn-primary manage-button" data-id="${row.id}" data-petition-id="${row.petition_id}">ดูรายละเอียด</button>`;
                                }
                                return buttonHtml;
                            }
                        }
    ...
    $('#follow_up_on_requests tbody').on('click', '.manage-button', function() {
        ...
        if (petition_id == 12) {
            pdfUrl = 'parent_permission_form_pdf.php?id=' + id;
        } else {
            // สร้าง URL ของหน้าอื่นที่ต้องการเปิด
            pdfUrl = 'request_payment_for_parcels_pdf?id=' + id;
        }
